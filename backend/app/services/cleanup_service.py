...
def _finish(base: dict, workspace_id: str, user_id: str, status: str, detail: str) -> dict:
    ...
    record = audit_repository.append(workspace_id, entry)
    logger.info(
...
def execute(
    *,
    ...
    try:
        ...
        return _finish(base, workspace_id, user_id, "unknown_account", str(exc))
    except PreconditionError as exc:
        return _finish(base, workspace_id, user_id, "precondition_failed", str(exc))
    except Exception as exc:  # AWS / unexpected error — still audited
        return _finish(base, workspace_id, user_id, "error", f"{type(exc).__name__}: {exc}")

    return _finish(base, workspace_id, user_id, "dry_run" if dry_run else "success", detail)
